...
UINT32 NextFatEntry(UINT32 Num, UINT32 *Ent)
{
    ...
    {
        e = VolReadSectors(v, (VPTR)FatSect, FatSector+(Num/128), 1);
        if(EFAIL(e))
		{
			DbgPrintf("NextFatEntryFailes");
			return e;
		}
        FatLoaded = (Num/128);
    }
...
UINT32 ReadCluster(VPTR Buffer, UINT32 Num)
{	UINT32 e;
    e = VolReadSectors(v, Buffer, DataSector+((Num-2)*BootSect.SectorsPerCluster), BootSect.SectorsPerCluster);
	if(EFAIL(e)) DbgPrintf("ReadCluster Failed");
	return e;
}
...
UINT32 ReadFile(UINT32 Clus, VPTR Buffer, UINT32 FileSize)
{
    ...
    while((Clus>=MinClus) && (Clus<=MaxClus))
    {
		...
		//if(EFAIL(e)) DbgPrintf("5");
        if(EFAIL(e)) return e;
		
        e = NextFatEntry(Clus, &Clus);
		//DbgPrintf("%d %d %d\n", Clus, MinClus, MaxClus);
		//if(EFAIL(e)) DbgPrintf("6");
        if(EFAIL(e)) return e;
        i++;
    ...
    {DbgPrintf("%d, %d\n", i*BootSect.SectorsPerCluster*512, FileSize);
        return EFAILED;
    }
}
...
UINT32 STDCALL Fat32ReadFile(VPTR Vol, char *Path, VPTR Buffer)
{
    ...
    v = Vol;
    e = ValidatePath(Path);
	if(EFAIL(e)) DbgPrintf("1");
    if(EFAIL(e)) return e;
    e = ReadFat32Info();
	if(EFAIL(e)) DbgPrintf("2");
    if(EFAIL(e)) return e;
    ...
    while(1)
    {
		...
		if(EFAIL(e)) DbgPrintf("3");
        if(EFAIL(e)) return e;
        if(Path[0]=='\0')
        {
            e = ReadFile(Dir, Buffer, Size);
			if(EFAIL(e)) DbgPrintf("4");
            return e;
        }
    }
}
