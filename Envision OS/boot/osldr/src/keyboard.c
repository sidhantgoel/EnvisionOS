...
void KbdCpuReset()
{
	BYTE Data;
	PicVectorDisable(1);
	KbdController(CTRL_DEACTIVATE);
	KbdController(CTRL_READOUTPORT);
	Data = KbdIn();
	Data &= 0xFE;
	KbdController(CTRL_WRITEOUTPORT);
	KbdCommand(Data, false);
	//Method 2
	KbdController(0xFE);
	DbgPrintf("Reset Failed");
}
...
int KbdProgram(int sc, int tp, int xlat)
{
	...
	KbdCommand(CMDB_EKI|CMDB_SYS|CMDB_DMS|(xlat ? CMDB_KCC : 0), false);
