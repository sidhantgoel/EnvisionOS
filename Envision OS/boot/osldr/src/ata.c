...
#define STATUS_CHK              0x01
#define STATUS_SERV             0x10
#define STATUS_DMRD             0x20
...
#define WaitDrvRdy(Base, Timeout)       AtaWaitStatus(Base, STATUS_DRDY|STATUS_BUSY, STATUS_DRDY, Timeout);
#define WaitReadyDrqOff(Base, Timeout)  AtaWaitStatus(Base, STATUS_BUSY|STATUS_DRQ, 0, Timeout)
#define WaitReadyDrq(Base, Timeout)     AtaWaitStatus(Base, STATUS_BUSY|STATUS_DRQ, STATUS_DRQ, Timeout)
#define WaitReady(Base, Timeout)        AtaWaitStatus(Base, STATUS_BUSY, 0, Timeout)
#define WaitDrq(Base, Timeout)          AtaWaitStatus(Base, STATUS_DRQ, STATUS_DRQ, Timeout)
#define WaitDrqOff(Base, Timeout)       AtaWaitStatus(Base, STATUS_DRQ, 0, Timeout)
...
void TryEject(AtaDevice *pDevice)
{
    ...
    DbgPrintf("Ejecting\n");
    while(inp(pDevice->Command+IDE_STATUS) & STATUS_BUSY);
    DbgPrintf("Ejecting2 %X\n", inp(pDevice->Command+IDE_STATUS));
    while((~inp(pDevice->Command+IDE_STATUS)) & STATUS_DRDY);
    DbgPrintf("Ejecting3\n");
    ...
    while(inp(pDevice->Command+IDE_STATUS) & STATUS_BUSY);
    DbgPrintf("Ejecting4\n");
    while((~inp(pDevice->Command+IDE_STATUS)) & STATUS_DRQ);
    DbgPrintf("Ejecting5\n");
...
UINT32 STDCALL AtaProbe()
{
	...
	for(i=0;i<2;i++)
	{
		...
		for(j=0;j<2;j++)
        {
            ...
            if(EFAIL(e)) continue;
            StorageCreate(pDevice->SerialNumber, (VPTR)pDevice, (pDevice->Flags & DEVICE_ATAPI) ? STORAGE_CDROM : STORAGE_HDD, &AtaDevFunc);
		}
	}
	return ESUCCESS;
}
